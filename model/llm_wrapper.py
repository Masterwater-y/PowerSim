...
class WrapperConfig:
    ...
    tiny_attn_dropout: float = 0.1
    tiny_rope_theta: float = 10000.0
    ...
    def __init__(self, cfg: WrapperConfig, hf_tokenizer):
        ...
        if cfg.tiny_transformer:
            ...
            tcfg = TinyTransformerConfig(
                ...
                max_len=cfg.max_len,
                dropout=cfg.tiny_dropout,
                attn_dropout=cfg.tiny_attn_dropout,
                pad_token_id=hf_tokenizer.pad_token_id or 0,
                rope_theta=cfg.tiny_rope_theta,
            )
            self.backbone = TinyTransformer(tcfg).to(model_dtype)
        ...
        # tiny 模式也保持这个口径，确保有效 embedding 参数量对齐 v22/v25a 11M 设定。
        self._unfreeze_new_embeddings(len(hf_tokenizer))
        self.head = PMURegressionHead(
            d_model, hidden=cfg.head_hidden,
            cpi_head_mode=cfg.cpi_head_mode).to(model_dtype)
        self.uop_encoder = UopEncoder(
            d_model, field_dim=cfg.uop_field_dim).to(model_dtype)
        self.side_proj = nn.Linear(cfg.side_feat_dim, d_model).to(model_dtype)
        ...
        self.local_proj = nn.Linear(d_model, d_model).to(model_dtype)
        nn.init.zeros_(self.local_proj.weight)
        nn.init.zeros_(self.local_proj.bias)
        self.local_bind_fuse = LocalBindingFuse(d_model).to(model_dtype)
        ...
        self.tstart_proj = nn.Linear(1, d_model).to(model_dtype)
