    ...
    for (cid, tid), rows in rows_by_key.items():
        ...
        for r in rows:
            ...
            if r['is_last_microop'] or not r['is_microop']:
                n_macro += 1
            valid = r['mispred_valid'] or (
                r['is_branch'] and (r['is_last_microop'] or not r['is_microop']))
            if valid:
                n_mp += r['mispred_hard']
        cycles = rc
