        ...
        with open(rf) as fr, open(lf) as fl_:
            for lr_, ll_ in zip(fr, fl_):
                ...
                mispred_valid = (
                    ...
                    and (int(jr.get('is_last_microop', 0)) > 0
                         or int(jr.get('is_microop', 0)) == 0)
                )
                mp_truth = int(jl.get('mispredicted', 0)) if mispred_valid else 0

                p = pred[key]
                fl_p = float(p['fetch_lat'])
                el_p = float(p['exec_lat'])
                mp_p = int(p['mispred_hard']) if mispred_valid else 0
