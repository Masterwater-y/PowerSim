...
class _Heads(nn.Module):
    ...
    fetch 采用 deterministic candidate group + residual hurdle：
      - candidate_fetch_group_head 由 functional trace 可复现规则给出
      - head_logit 只在 candidate 上表示该 candidate 是否承接真实 gap
      - 非 candidate row 在训练/推理中强制 fetch=0
    """
...
class TaoCoreTransformer(nn.Module):
    ...
    def compute_loss(self, batch: Dict, out: Dict) -> Dict[str, torch.Tensor]:
        ...
        mse_f_cons = torch.tensor(0.0, device=out['exec_lat'].device,
                                  dtype=out['exec_lat'].dtype)
        if 'head' in batch:
            candidate_mask = batch.get('candidate_fetch_group_head')
            if candidate_mask is None:
                candidate_mask = torch.ones_like(batch['head'])
            candidate_mask = candidate_mask > 0.5
            fetch_target = batch.get('candidate_fetch_lat', batch['fetch_lat'])
            head_target = (fetch_target > 0.0).to(fetch_target.dtype)
            head_mask = candidate_mask & (head_target > 0.5)
            if torch.any(head_mask):
                mse_f_pos = F.mse_loss(out['fetch_lat'][head_mask],
                                       fetch_target[head_mask])
            fetch_soft = out['fetch_lat_soft'] * candidate_mask.to(out['fetch_lat_soft'].dtype)
            mse_f_cons = F.mse_loss(fetch_soft, fetch_target)
        else:
            mse_f_pos = F.mse_loss(out['fetch_lat'], batch['fetch_lat'])
            mse_f_cons = mse_f_pos

        bce_fetch_tail = torch.tensor(0.0, device=out['exec_lat'].device,
                                      dtype=out['exec_lat'].dtype)
        fetch_tail_raw_mae = torch.tensor(0.0, device=out['exec_lat'].device,
                                          dtype=out['exec_lat'].dtype)
        fetch_decomp = torch.tensor(0.0, device=out['exec_lat'].device,
                                    dtype=out['exec_lat'].dtype)
        fetch_decomp_cons = torch.tensor(0.0, device=out['exec_lat'].device,
                                         dtype=out['exec_lat'].dtype)
        ...
        if 'head' in batch and 'head_logit' in out:
            ...
            if candidate_mask is not None:
                ...
                if torch.any(candidate_mask):
                    fetch_target = batch.get('candidate_fetch_lat', batch['fetch_lat'])
                    head_target = (fetch_target > 0.0).to(out['head_logit'].dtype)
                    bce_h = F.binary_cross_entropy_with_logits(
                        out['head_logit'][candidate_mask],
                        head_target[candidate_mask],
                        pos_weight=head_pw)
            else:
                bce_h = F.binary_cross_entropy_with_logits(
