...
int main(int argc, char **argv)
{
    if (argc != 4 && argc != 5) {
        std::fprintf(stderr,
            ...
            "[--mode=oracle-replay|--mode=functional-only]\n",
            argv[0]);
        return 2;
    }
    ...
    if (argc == 5) {
        ...
        if (mode_arg == "--mode=functional-only" || mode_arg == "--functional-only") {
            functional_only = true;
        } else if (mode_arg == "--mode=oracle-replay") {
            functional_only = false;
        } else {
            std::fprintf(stderr, "[ref_sim] unknown mode argument: %s\n", argv[4]);
            return 2;
        }
    }
    ...
    while (std::getline(fin, line)) {
        ...
        if (et == "evict") {
            ...
            if (functional_only) {
                ...
                ev.size           = uint16_t(getU64(line, "size", 64));
                DSideOracle d = sim.step(ev);
                coh_pred = unsigned(d.coh_oracle);
            } else {
                coh_pred = unsigned(getU64(line, "coh_oracle"));
            }
            fout << "{\"seq\":" << seq
                 << ",\"core_id\":" << core_id
                 << ",\"event_type\":\"request\""
