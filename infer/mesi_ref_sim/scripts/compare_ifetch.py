#!/usr/bin/env python3
"""compare_ifetch.py — V9.5 i-cache 4 字段 bit-exact 校验。

oracle 端 ifetch 行字段：i_path_class / i_coh_oracle / i_mesi_before
ref_sim 端 ifetch 行字段：i_path_class / i_coh_oracle / i_mesi_before /
                          i_oracle_source

对齐 key: (seq, core_id)；逐字段 0-diff，否则记录 mismatch。

V9.6 ROI 边界过滤（与 dside 的 oracle_source==0 策略对齐）：
    在 V9.6 ROI 模式下，probe 内部 line_states_ 走 always-update，
    可在 ROI 起点已记得 ROI 外的 fill；但 ref_sim 只重放 ROI 内的
    mem_events，对 "在 ROI 内首次接触该 cacheline" 的 ifetch 行必然
    判定为 cold（path=DRAM, coh=cold），与 oracle 报 hot 形成必然失配。
    
    解法：扫 mem_events 时维护已见 cacheline 集合；若一条 ifetch 是
    该 cl 在该流中的首次出现，则跳过严格 eval（计为 roi_edge_skipped）。
    V9.5 模式（require_roi=False）时，启动期 fill 也在流里，集合在
    ROI 外就被填好，该过滤天然不生效，行为与旧版一致。

用法:
    compare_ifetch.py <mem_events.jsonl> <pred.jsonl>
"""
import collections
import json
import os
import sys

FIELDS = ('i_path_class', 'i_coh_oracle', 'i_mesi_before')


def main():
    if len(sys.argv) != 3:
        print("Usage: compare_ifetch.py <mem_events.jsonl> <pred.jsonl>",
              file=sys.stderr)
        sys.exit(2)
    ev_path, pr_path = sys.argv[1], sys.argv[2]
    preds = {}
    with open(pr_path) as f:
        for ln in f:
            if not ln.strip().startswith('{'):
                continue
            r = json.loads(ln)
            if r.get('event_type') != 'ifetch':
                continue
            preds[(r['seq'], r.get('core_id', 0))] = r

    n = matched = mismatched = roi_edge_skipped = 0
    per_field_mismatch = collections.Counter()
    samples = []
    seen_cl = set()  # mem_events 流中已观察过的 cacheline_addr 集合
    with open(ev_path) as f:
        for ln in f:
            if not ln.strip().startswith('{'):
                continue
            r = json.loads(ln)
            cl = r.get('cacheline_addr')
            ev_type = r.get('event_type')
            if ev_type != 'ifetch':
                # 任意 dside / evict / prefetch 触及 → 该 cl 已在 ROI 内被见过
                if cl is not None:
                    seen_cl.add(cl)
                continue
            n += 1
            # V9.6 ROI 边界过滤：cl 在 ROI 内首次出现 → ROI 外预热，跳过严格 eval
            first_in_roi = cl is not None and cl not in seen_cl
            if cl is not None:
                seen_cl.add(cl)
            if first_in_roi:
                roi_edge_skipped += 1
                continue
            key = (r['seq'], r.get('core_id', 0))
            p = preds.get(key)
            if p is None:
                mismatched += 1
                per_field_mismatch['__missing__'] += 1
                if len(samples) < 50:
                    samples.append((key, r, None))
                continue
            ok = True
            for fld in FIELDS:
                if int(r.get(fld, -1)) != int(p.get(fld, -2)):
                    per_field_mismatch[fld] += 1
                    ok = False
            if ok:
                matched += 1
            else:
                mismatched += 1
                if len(samples) < 50:
                    samples.append((key, r, p))

    strict_n = n - roi_edge_skipped
    rate = (matched / strict_n * 100) if strict_n else 0.0
    print(f"ifetch events       : {n}")
    print(f"roi_edge_skipped    : {roi_edge_skipped}  "
          f"(cl 在 ROI 内首次出现, ROI 外预热)")
    print(f"strict-eval rows    : {strict_n}")
    print(f"matched             : {matched}  ({rate:.4f}%)")
    print(f"mismatched          : {mismatched}")
    print(f"per-field misses    : {dict(per_field_mismatch)}")
    if samples:
        out = os.path.join(os.path.dirname(pr_path) or '.', 'mismatch.ifetch.csv')
        with open(out, 'w') as g:
            g.write("seq,core,cl,o_path,o_coh,o_mesi,p_path,p_coh,p_mesi\n")
            for key, r, p in samples:
                seq, cid = key
                cl = r.get('cacheline_addr', 0)
                o = (r.get('i_path_class', -1), r.get('i_coh_oracle', -1),
                     r.get('i_mesi_before', -1))
                pp = (p.get('i_path_class', -1) if p else -1,
                      p.get('i_coh_oracle', -1) if p else -1,
                      p.get('i_mesi_before', -1) if p else -1)
                g.write(f"{seq},{cid},{cl},"
                        f"{o[0]},{o[1]},{o[2]},"
                        f"{pp[0]},{pp[1]},{pp[2]}\n")
        print(f"mismatch samples  -> {out}")
    if mismatched:
        sys.exit(1)


if __name__ == '__main__':
    main()
