...
_CORE_NUMCYC = re.compile(r"cores(\d+)\.core\.numCycles\s+([0-9.]+)")
_CORE_INSTS = re.compile(
    r"cores(\d+)\.core\.commitStats0\.numInsts\s+([0-9.]+)")
...
def main():
    ...
    if os.path.isfile(head_pt):
        ...
        if "new_token_embedding" in sd:
            ...
            print("[WARN] ckpt 缺 new_token_embedding，推理用随机初始化，结果无效！",
                  file=sys.stderr)
    model.eval()
    ...
    if jsonl_total != len(ds):
        print(f"[FATAL] cache 丢弃了样本：jsonl={jsonl_total} cache={len(ds)}，"
              f"行号与 cache 下标不对齐，无法按 workload 子集化。\n"
              f"请用与 cache 相同的 max-len（当前 {args.max_len}），"
              f"或重建 cache 后再评估。", file=sys.stderr)
        sys.exit(1)
    targets = []
    ...
    print("=" * 78)
    print(f"全局 CPI 验证  (cache={ds.mode}, total_windows={len(ds)})")
    print("=" * 78)
    ...
    for name, stats_path in targets:
        ...
        if not rows:
            print(f"[skip] {name}: 在 {args.data} 中无样本")
            continue
        if args.max_windows:
            rows = rows[:args.max_windows]
        sub = torch.utils.data.Subset(ds, rows)
        ...
        sum_cyc_label = 0.0
        sum_macro = 0.0
        per_window_ape = []  # 每核每窗口 CPI 的相对误差（vs label）
        with torch.no_grad():
            for b in dl:
                ...
                per_window_ape.extend(ape[m].tolist())
        ...
        win_mape = float(np.mean(per_window_ape)) if per_window_ape else float("nan")

        print(f"\n## {name}   (windows={len(rows)}, cores≈{int(macro.shape[1])})")
        print(f"  全局CPI  pred ={cpi_pred_g:.4f}")
        print(f"  全局CPI  label={cpi_label_g:.4f}   (窗口标签聚合)")
        print(f"  全局CPI  gem5 ={cpi_gem5:.4f}   (stats.txt 全程)")
        print(f"  误差  pred vs label = {err_vs_label*100:.2f}%   <- 纯模型预测能力")
        print(f"  误差  pred vs gem5  = {err_vs_gem5*100:.2f}%   <- 端到端(含窗口采样偏差)")
        print(f"  参考  label vs gem5 = {label_vs_gem5*100:.2f}%   <- 窗口采样本身的偏差")
        print(f"  per-window CPI MAPE = {win_mape*100:.2f}%")
        summary.append({
            "workload": name,
