...
jsonl 流逐条预测 (fetch_latency, execution_latency, mispredicted, head)。
...
要点：
  ...
  4. 推理批量按行序滑动窗口（不打乱）；mispredicted / head 输出 sigmoid
     概率与硬阈值。
  5. fetch_lat / exec_lat 是 log1p(cycle)，需要 expm1 反变换；其中
     fetch_lat 使用 head_hard 做硬门控，非 head 强制输出 0。
  6. CPU bf16 autocast。

输出 jsonl 行 schema：
  {"workload":..,"core_id":..,"thread_id":..,"micro_seq":..,
   "fetch_lat":<float-cycles>, "fetch_lat_pos":<float-cycles>,
   "exec_lat":<float-cycles>, "mispred_prob":<float>, "mispred_hard":0|1,
