...
class ParquetWindowDataset:
    ...
    def __getitem__(self, idx: int) -> Dict[str, np.ndarray]:
        ...
        # ---- features
        # 1) bool/small int 直接取；i_oracle_source 不送入模型
        #    （恒值 / 分布偏移），从 feat 中剔除
        feat = {}
        ...
        if self.spec.label_log1p:
            lbl['fetch_latency_t'] = np.log1p(max(float(lbl['fetch_latency']), 0.0)).astype(np.float32)
            lbl['execution_latency_t'] = np.log1p(max(float(lbl['execution_latency']), 0.0)).astype(np.float32)
        else:
