...
def latency_bucket(arr: np.ndarray, n_bins: int = 8) -> np.ndarray:
    ...
    a = np.maximum(arr.astype(np.float64), 0.0)
    a = np.log1p(a)
    cap = np.log1p(4096.0)
    a = np.minimum(a, cap)
...
def dedup_partition(in_path: Path, out_path: Path, ctx_len: int,
                    ...
                    protect_latency_quantile: float,
                    log) -> tuple[int, int, int, int, dict]:
    """返回 (n_in, n_out, n_pos_protected, n_tail_protected, tail_thresholds)。"""
    ...
    fetch = tbl['fetch_latency'].to_numpy(zero_copy_only=False).astype(np.float64)
    exe = tbl['execution_latency'].to_numpy(zero_copy_only=False).astype(np.float64)
    tail_mask = np.zeros(n, dtype=bool)
    tail_thresholds = {}
    if protect_latency_quantile > 0.0:
        q = float(np.clip(protect_latency_quantile, 0.0, 1.0))
        f_thr = float(np.quantile(fetch, q))
        e_thr = float(np.quantile(exe, q))
        tail_mask = (fetch >= f_thr) | (exe >= e_thr)
        tail_thresholds = {
            'quantile': q,
            'fetch_latency': f_thr,
            'execution_latency': e_thr,
        }

    # 去重：稳定保留首次出现
    preserve_mask = tail_mask.copy()
    if protect_positives:
        preserve_mask |= (mis == 1)
    if preserve_mask.any():
        neg_mask = ~preserve_mask
        keep = np.zeros(n, dtype=bool)
        keep[preserve_mask] = True
        ...
        keep[first] = True
        n_protected = 0
    n_tail_protected = int(tail_mask.sum())
    ...
    return n, new_tbl.num_rows, n_protected, n_tail_protected, tail_thresholds
...
def main():
    ...
    ap.add_argument('--protect-latency-quantile', type=float, default=0.99,
                    help='>= 该分位数的 fetch/exec latency 样本不参与去重；'
                         '默认 0.99，<=0 表示关闭。')
    args = ap.parse_args()
    ...
    t0 = time.time()
    summary = {}
    total_in = total_out = total_prot = total_tail_prot = 0
    for src in parts:
        ...
        log(f'[dedup] {w}')
        n_in, n_out, n_prot, n_tail_prot, tail_thresholds = dedup_partition(
            src, dst, args.context_len,
            protect_positives=not args.no_protect_positives,
            latency_bins=args.latency_bins,
            protect_latency_quantile=args.protect_latency_quantile,
            log=log)
        ratio = n_out / max(n_in, 1)
        log(f'    kept = {n_out:,} / {n_in:,}  ({ratio:.3%}; '
            f'protected positives = {n_prot:,}; '
            f'protected tail = {n_tail_prot:,})')
        summary[w] = {
            'in': n_in,
            'out': n_out,
            'protected_pos': n_prot,
            'protected_tail': n_tail_prot,
            'tail_thresholds': tail_thresholds,
        }
        total_in += n_in
        total_out += n_out
        total_prot += n_prot
        total_tail_prot += n_tail_prot
    ...
    m['dedup_protect_latency_quantile'] = args.protect_latency_quantile
    ...
    log(f'       protected latency tail = {total_tail_prot:,}')
