...
from latency_units import (
    latency_unit_metadata,
    load_uarch_profile,
    ticks_to_cycles,
)
...
def derive_thread_signals(rec_t: pa.Table, lab_t: pa.Table, uarch_profile):
    """对一个 core 的 records/labels 算:
       - fetch_lat (float64 cycles, 全行)
       - exec_lat  (float64 cycles, 全行)
       - is_head   (int8, 全行)
       - after_mispred_k4 (int8, 全行)
    ...
    is_branch = rec_t.column('is_branch').to_numpy(zero_copy_only=False) > 0
    is_microop = rec_t.column('is_microop').to_numpy(zero_copy_only=False) > 0
    is_last_microop = rec_t.column('is_last_microop').to_numpy(zero_copy_only=False) > 0
    mispred = lab_t.column('mispredicted').to_numpy(zero_copy_only=False) > 0
    branch_event = is_branch & (is_last_microop | (~is_microop)) & mispred
    n = len(tid_arr)
    fetch_lat_ticks = np.zeros(n, dtype=np.int64)
    is_head = np.zeros(n, dtype=np.int8)
    after_mispred_k4 = np.zeros(n, dtype=np.int8)
    exec_lat_ticks = (rt_arr - ft_arr).astype(np.int64)
    ...
    for k, tid in enumerate(unique_tids):
        ...
        fetch_lat_ticks[idx] = diff
        head = np.zeros(nt, dtype=np.int8)
        head[0] = 1
        if nt > 1:
            head[1:] = (diff[1:] > 0).astype(np.int8)
        is_head[idx] = head
        ev = branch_event[idx].astype(np.int32, copy=False)
        prefix = np.empty(nt + 1, dtype=np.int32)
        prefix[0] = 0
        np.cumsum(ev, out=prefix[1:])
        row = np.arange(nt, dtype=np.int64)
        lo = np.maximum(row - 4, 0)
        # prefix[row] excludes the current row, so a branch row is not charged
        # with its own recovery gap.
        after_mispred_k4[idx] = ((prefix[row] - prefix[lo]) > 0).astype(np.int8)
        window_per_tid[int(tid)] = idx

    fetch_lat = ticks_to_cycles(fetch_lat_ticks, uarch_profile)
    exec_lat = ticks_to_cycles(exec_lat_ticks, uarch_profile)
    return fetch_lat, exec_lat, is_head, after_mispred_k4, window_per_tid
...
def load_timing_functional_backend(tao_cpu_sim_root: str):
    root = os.path.abspath(tao_cpu_sim_root)
    infer_src = os.path.join(root, 'src', '04_infer')
    if infer_src not in sys.path:
        sys.path.insert(0, infer_src)
    try:
        from driver.timing_functional_refsim import (  # type: ignore
            TimingFunctionalBackend,
            TimingFunctionalConfig,
        )
    except Exception as e:
        raise RuntimeError(
            f"failed to import timing-functional backend from {infer_src}: {e}"
        ) from e
    return TimingFunctionalBackend, TimingFunctionalConfig


def apply_timing_functional_features(core_data, uarch_profile,
                                     tao_cpu_sim_root: str) -> None:
    """Override Ruby oracle uarch columns with deploy-side functional features."""
    TimingFunctionalBackend, TimingFunctionalConfig = load_timing_functional_backend(
        tao_cpu_sim_root)
    backend = TimingFunctionalBackend(uarch_profile, TimingFunctionalConfig())

    per_core_attrs = {}
    for cid in sorted(core_data):
        rec_t = core_data[cid]['rec']
        n = rec_t.num_rows
        cols = {name: rec_t.column(name).to_numpy(zero_copy_only=False)
                for name in (
                    'thread_id', 'micro_seq', 'paddr', 'is_load', 'is_store',
                    'is_atomic', 'size',
                )}
        attrs_by_field = {
            k: np.zeros(n, dtype=np.int16) for k in TIMING_FUNCTIONAL_FIELDS
        }
        for i in range(n):
            is_load = bool(cols['is_load'][i])
            is_store = bool(cols['is_store'][i]) or bool(cols['is_atomic'][i])
            if is_load or is_store:
                attrs = backend.on_mem_access(
                    int(cid),
                    int(cols['paddr'][i]),
                    is_store,
                    int(cols['size'][i]),
                    int(cols['micro_seq'][i]),
                    int(cols['thread_id'][i]),
                )
            else:
                attrs = backend._zero_attrs()
            for k in TIMING_FUNCTIONAL_FIELDS:
                attrs_by_field[k][i] = int(attrs.get(k, 0))
        per_core_attrs[cid] = attrs_by_field

    for cid, attrs_by_field in per_core_attrs.items():
        rec_t = core_data[cid]['rec']
        names = list(rec_t.schema.names)
        for k in TIMING_FUNCTIONAL_FIELDS:
            arr = pa.array(attrs_by_field[k], type=pa.int16())
            if k in names:
                rec_t = rec_t.set_column(names.index(k), k, arr)
            else:
                rec_t = rec_t.append_column(k, arr)
                names.append(k)
        core_data[cid]['rec'] = rec_t


# ============================================================ 全 workload 收集
def collect_workload(run_dir: str, head_skip: float, tail_skip: float,
                     ctx_warmup: int, uarch_profile,
                     business_cores=(0, 1, 2, 3),
                     feature_generator='oracle',
                     tao_cpu_sim_root=None):
    """返回:
    ...
    for rf in rec_files:
        ...
        fl, el, ih, am, w_per_tid = derive_thread_signals(
            rec_t, lab_t, uarch_profile)
        core_data[cid] = {
            'rec': rec_t, 'lab': lab_t, 'fl': fl, 'el': el, 'ih': ih,
            'am': am,
        }
        for tid, gidx in w_per_tid.items():
            ...
            if wnd is None:
                continue
            windows[(cid, tid)] = wnd
    if feature_generator == 'timing-functional':
        print("  feature_generator=timing-functional: overriding uarch columns",
              file=sys.stderr)
        apply_timing_functional_features(
            core_data, uarch_profile, tao_cpu_sim_root)
...
def make_flat_schema():
    ...
    fields += [
        ...
        ('fetch_latency', pa.float64()),
        ('execution_latency', pa.float64()),
        ('is_fetch_group_head', pa.int8()),
        ('candidate_fetch_group_head', pa.int8()),
        ('candidate_fetch_latency', pa.float64()),
...
def derive_candidate_targets_for_core(rec_t: pa.Table, fetch_lat: np.ndarray):
    """Derive candidate rows and carry non-candidate fetch gaps forward."""
    n = rec_t.num_rows
    tid_arr = rec_t.column('thread_id').to_numpy(
        zero_copy_only=False).astype(np.int64)
    macro_arr = rec_t.column('macro_pc').to_numpy(
        zero_copy_only=False).astype(np.uint64)
    is_micro_arr = rec_t.column('is_microop').to_numpy(
        zero_copy_only=False).astype(np.int64)
    is_last_arr = rec_t.column('is_last_microop').to_numpy(
        zero_copy_only=False).astype(np.int64)
    is_branch_arr = rec_t.column('is_branch').to_numpy(
        zero_copy_only=False).astype(np.int64)
    is_serialize_arr = rec_t.column('is_serialize').to_numpy(
        zero_copy_only=False).astype(np.int64)
    fetch_arr = np.maximum(np.asarray(fetch_lat, dtype=np.float64), 0.0)

    candidate_head = np.zeros(n, dtype=np.int8)
    candidate_fetch_lat = np.zeros(n, dtype=np.float64)
    prev_macro = {}
    prev_i_cl = {}
    prev_is_micro = {}
    prev_is_last = {}
    ...
    for i in range(n):
        key = int(tid_arr[i])
        pm = prev_macro.get(key)
        pi = prev_i_cl.get(key)
        p_micro = prev_is_micro.get(key, 0)
        p_last = prev_is_last.get(key, 0)
        cur_macro = int(macro_arr[i])
        cur_i_cl = cur_macro >> 6
        macro_head = (pm is None or p_micro == 0 or p_last == 1
                      or cur_macro != pm)
        i_group_head = (pi is None or cur_i_cl != pi)
        is_candidate = int(
            macro_head or i_group_head
            or int(is_branch_arr[i]) > 0
            or int(is_serialize_arr[i]) > 0)
        candidate_head[i] = is_candidate
        if is_candidate:
            candidate_fetch_lat[i] = carry[key] + fetch_arr[i]
            carry[key] = 0.0
        else:
            carry[key] += fetch_arr[i]
        prev_macro[key] = cur_macro
        prev_i_cl[key] = cur_i_cl
        prev_is_micro[key] = int(is_micro_arr[i])
        prev_is_last[key] = int(is_last_arr[i])
    return candidate_head, candidate_fetch_lat
...
def assemble_flat_table(workload: str, core_data, picks):
    ...
    fl_list, el_list, ih_list, am_list = [], [], [], []
    cand_head_list, cand_lat_list = [], []
    ...
    for cid in cid_order:
        ...
        am_list.append(core_data[cid]['am'][all_idx])
        if 'candidate_head' not in core_data[cid]:
            candidate_head, candidate_fetch_lat = derive_candidate_targets_for_core(
                core_data[cid]['rec'], core_data[cid]['fl'])
            core_data[cid]['candidate_head'] = candidate_head
            core_data[cid]['candidate_fetch_lat'] = candidate_fetch_lat
        cand_head_list.append(core_data[cid]['candidate_head'][all_idx])
        cand_lat_list.append(core_data[cid]['candidate_fetch_lat'][all_idx])
    ...
    am_arr = np.concatenate(am_list).astype(np.int8, copy=False)
    candidate_head = np.concatenate(cand_head_list).astype(np.int8, copy=False)
    candidate_fetch_lat = np.concatenate(cand_lat_list).astype(np.float64, copy=False)
    ...
    cols['fetch_latency'] = pa.array(fl_arr, type=pa.float64())
    cols['execution_latency'] = pa.array(el_arr, type=pa.float64())
    cols['is_fetch_group_head'] = pa.array(ih_arr, type=pa.int8())
    cols['candidate_fetch_group_head'] = pa.array(candidate_head, type=pa.int8())
    cols['candidate_fetch_latency'] = pa.array(candidate_fetch_lat, type=pa.float64())
    fetch_p95 = float(np.quantile(fl_arr, 0.95)) if n else 0.0
    residual_mask = (fl_arr >= fetch_p95) & (am_arr == 0)
    after_lat = np.where(am_arr > 0, fl_arr, 0.0)
    residual_lat = np.where(residual_mask, fl_arr, 0.0)
    base_lat = np.maximum(fl_arr - after_lat - residual_lat, 0.0)
    cols['fetch_base_latency'] = pa.array(base_lat, type=pa.float64())
    cols['fetch_after_mispred_latency'] = pa.array(after_lat, type=pa.float64())
    cols['fetch_residual_tail_latency'] = pa.array(residual_lat, type=pa.float64())
    cols['fetch_after_mispred_k4'] = pa.array(am_arr, type=pa.int8())
...
def emit_legacy_jsonl(workload: str, core_data, picks, out_path: str):
    ...
    n_emitted = 0
    for cid in {int(x[0]) for x in picks}:
        if 'candidate_head' not in core_data[cid]:
            candidate_head, candidate_fetch_lat = derive_candidate_targets_for_core(
                core_data[cid]['rec'], core_data[cid]['fl'])
            core_data[cid]['candidate_head'] = candidate_head
    ...
    with open(out_path, 'w') as out_fp:
        ...
        for (cid, tid, gidx, pidx) in picks:
            ...
            ih = core_data[cid]['ih']
            am = core_data[cid]['am']
            fetch_p95 = float(np.quantile(fl[gidx], 0.95)) if len(gidx) else 0.0
            ...
            for j, (r, l) in enumerate(zip(sub_rec, sub_lab)):
                gi = int(gidx[j])
                after_mispred = int(am[gi]) > 0
                residual_tail = (float(fl[gi]) >= fetch_p95) and not after_mispred
                fetch_after = float(fl[gi]) if after_mispred else 0.0
                fetch_residual = float(fl[gi]) if residual_tail else 0.0
                fetch_base = max(float(fl[gi]) - fetch_after - fetch_residual, 0.0)
                candidate_head = int(core_data[cid]['candidate_head'][gi])
                candidate_fetch = float(core_data[cid]['candidate_fetch_lat'][gi])
                sample = {
                    "meta": {
                        "workload": workload,
                        "core_id": cid,
                    ...
                    "labels": {
                        ...
                        "fetch_latency": float(fl[gi]),
                        "execution_latency": float(el[gi]),
                        "is_fetch_group_head": int(ih[gi]),
                        "candidate_fetch_group_head": candidate_head,
                        "candidate_fetch_latency": candidate_fetch,
                        "fetch_base_latency": fetch_base,
                        "fetch_after_mispred_latency": fetch_after,
                        "fetch_residual_tail_latency": fetch_residual,
                        "fetch_after_mispred_k4": int(am[gi]),
                    },
                }
...
def main():
    ...
    ap.add_argument('--uarch-profile', action='append', default=[],
                    metavar='NAME=PATH',
                    help='可选：指定 workload 对应的 uarch_profile.json；'
                         '默认读取 run_dir/uarch_profile.json。所有 workload '
                         'profile 必须一致，防止混用不同 gem5 µarch 参数。')
    ...
    ap.add_argument('--feature-generator',
                    choices=('oracle', 'timing-functional'),
                    default=os.environ.get('FEATURE_GENERATOR', 'oracle'),
                    help='oracle=保留 records.micro 的 Ruby oracle 字段；'
                         'timing-functional=用 deploy-side functional generator '
                         '覆盖 d/i-side uarch_context 字段。')
    ap.add_argument('--tao-cpu-sim-root',
                    default=os.environ.get(
                        'TAO_CPU_SIM_ROOT',
                        os.environ.get('TAO_ROOT', os.getcwd())),
                    help='timing-functional backend 所在 tao_cpu_sim 根目录。')
    ...
    explicit_profiles = {}
    for item in args.uarch_profile:
        if '=' not in item:
            sys.exit(f"--uarch-profile 必须形如 NAME=PATH，got: {item}")
        name, path = item.split('=', 1)
        explicit_profiles[name] = path

    profiles = {}
    profile_refs = {}
    for name, dir_ in runs:
        prof_path = explicit_profiles.get(
            name, os.path.join(dir_, 'uarch_profile.json'))
        try:
            prof = load_uarch_profile(prof_path)
        except Exception as e:
            sys.exit(f"[sample][FATAL] failed to load uarch profile for "
                     f"{name}: {e}")
        profiles[name] = prof
        # source 字段可能记录 pre/post-sim 来源，不参与统一 µarch 判等。
        prof_cmp = {k: v for k, v in prof.items()
                    if k not in ('source', '_latency_unit')}
        profile_refs[name] = json.dumps(prof_cmp, sort_keys=True,
                                        separators=(',', ':'))
        meta = latency_unit_metadata(prof)
        print(f"[uarch] {name:20s} profile={prof_path} "
              f"freq_ghz={meta['freq_ghz']} "
              f"ticks_per_cycle={meta['ticks_per_cycle']:.9g}",
              file=sys.stderr)
    if len(set(profile_refs.values())) > 1:
        sys.exit("[sample][FATAL] uarch_profile mismatch across workloads; "
                 "请使用同一份 uarch_profile.json 生成/采样数据集。")
    unit_meta = latency_unit_metadata(next(iter(profiles.values())))
    ...
    for name, d in runs:
        core_data, windows = collect_workload(
            ...
            args.context_warmup_skip, profiles[name],
            business_cores=business_cores,
            feature_generator=args.feature_generator,
            tao_cpu_sim_root=args.tao_cpu_sim_root)
    ...
    if not args.legacy_jsonl_out:
        schema = schema.with_metadata({
            ...
            b'latency_unit_meta':
                json.dumps(unit_meta, separators=(',', ':')).encode('utf-8'),
        })
