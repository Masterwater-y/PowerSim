...
from latency_units import load_uarch_profile, ticks_to_cycles
...
def build_one_core(workload, core_id, records_path, labels_path,
                   out_fp, stats, uarch_profile, roi_warmup_skip=0):
    """V9.5：单源 detailed 投影。
    ...
    with open(records_path) as fr, open(labels_path) as fl:
        for lr, ll in zip(fr, fl):
            ...
            ct = jl['commit_tick']
            rt = jl['ready_tick']
            exec_lat_ticks = rt - ft   # ready - fetch，恒 >= 0
            assert exec_lat_ticks >= 0, \
            ...
            if key not in prev_fetch:
                ...
                fetch_lat_ticks = ft - prev_fetch[key]
                assert fetch_lat_ticks >= 0, \
                    f"fetch_tick 回退! tid={tid} mseq={mseq} " \
                    f"prev_ft={prev_fetch[key]} ft={ft}"
                # V9.7 方案 B：fetch_tick 跳变 ⟺ 新 fetch cycle 的领头
                is_head = 1 if fetch_lat_ticks > 0 else 0
                fetch_clock_acc[key] += fetch_lat_ticks
                ready_acc[key] = max(ready_acc[key],
                                     fetch_clock_acc[key] + exec_lat_ticks)
                if rt > max_ready[key]:
                    max_ready[key] = rt
            prev_fetch[key] = ft
            last_commit[key] = ct
            sum_lat[key][0] += fetch_lat_ticks
            sum_lat[key][1] += exec_lat_ticks
            total_count[key] += 1
            fetch_lat = float(ticks_to_cycles(fetch_lat_ticks, uarch_profile))
            exec_lat = float(ticks_to_cycles(exec_lat_ticks, uarch_profile))
...
def main():
    ...
    ap.add_argument('--uarch-profile', default=None,
                    help='uarch_profile.json 路径；默认读取 detailed-dir 的父目录'
                         '/uarch_profile.json。latency 标签会从 gem5 tick 转为 cycle。')
    ...
    prof_path = args.uarch_profile
    if prof_path is None:
        prof_path = os.path.join(os.path.dirname(os.path.abspath(args.detailed_dir)),
                                 'uarch_profile.json')
    try:
        uarch_profile = load_uarch_profile(prof_path)
    except Exception as e:
        sys.exit(f"[build_micro_dataset][FATAL] failed to load uarch profile: {e}")
