...
2. **oracle 字段是 µop 输入属性，不是预测目标**。
   ...
   [mesi_ref_sim](file://MTAO/taogen/mesi_ref_sim) 现场重放产生。
...
实现位置：[gem5_patches/src/cpu/o3/probe](file://MTAO/taogen/gem5_patches/src/cpu/o3/probe)
([tao_trace.hh](file://MTAO/taogen/gem5_patches/src/cpu/o3/probe/tao_trace.hh) /
 [tao_trace.cc](file://MTAO/taogen/gem5_patches/src/cpu/o3/probe/tao_trace.cc) /
 [TaoTrace.py](file://MTAO/taogen/gem5_patches/src/cpu/o3/probe/TaoTrace.py))
...
[configs/run_mt_mvp.py](file://MTAO/taogen/configs/run_mt_mvp.py)：
...
[configs/single_core_arch_A.json](file://MTAO/taogen/configs/single_core_arch_A.json)：
8-wide O3、ROB=192，作为单核冒烟基准。

入口脚本：
- [scripts/run_experiment.sh](file://MTAO/taogen/scripts/run_experiment.sh) 一键 gem5 → ref_sim → 17/17 bit-exact + PMU 对比；
- [configs/step5_run_4workloads.sh](file://MTAO/taogen/configs/step5_run_4workloads.sh) / [step5_l3_4mib_smoke.sh](file://MTAO/taogen/configs/step5_l3_4mib_smoke.sh)。

### 1.3 4 个 workload（覆盖不同压力面）

| Workload | 压力面 | 源码 |
|---|---|---|
| W1 mt_compute_int | 纯 INT 算力 | [mt_compute_int.c](file://MTAO/taogen/workloads/mt_compute_int/mt_compute_int.c) |
| W2 mt_chase_dram  | 随机 pointer-chase，逼出 DRAM/LLC miss | [mt_chase_dram.c](file://MTAO/taogen/workloads/mt_chase_dram/mt_chase_dram.c) |
| W3 mt_micro_coh   | 轻量 coherence 触发 | [mt_micro_coh.c](file://MTAO/taogen/workloads/mt_micro_coh/mt_micro_coh.c) |
| W4 mt_coh_stress  | 重 coherence + 同 line 写穿 | [mt_coh_stress.c](file://MTAO/taogen/workloads/mt_coh_stress/mt_coh_stress.c) |

### 1.4 共享 µarch 模型 + ref_sim

为保证训练 oracle 与部署期 oracle 行为完全一致，两侧共享同一份配置和缓存模拟器：

- [shared/uarch_profile.hh](file://MTAO/taogen/shared/uarch_profile.hh)：手写 JSON 加载器（schema v2，`CacheCfg / TlbCfg / WalkerCfg / MshrCfg`）。
- [shared/lru_banked.hh](file://MTAO/taogen/shared/lru_banked.hh)：`BankedSetAssocLRU / TlbSim / PageWalkSim / MshrTracker`。
- [mesi_ref_sim](file://MTAO/taogen/mesi_ref_sim)：参考 MESI 状态机重放器，做 17/17 bit-exact 校验。
...
入口脚本：
- [scripts/build_3m_dataset.sh](file://MTAO/taogen/scripts/build_3m_dataset.sh)
- [scripts/pack_3m.sh](file://MTAO/taogen/scripts/pack_3m.sh)

### 2.2 Parquet schema（与训练侧严格绑定）

实现见 [tools/pack_to_parquet.py](file://MTAO/taogen/tools/pack_to_parquet.py) 与
[ml/dataset.py](file://MTAO/taogen/ml/dataset.py)。
...
[tools/subsample_dataset.py](file://MTAO/taogen/tools/subsample_dataset.py)：
...
实现：[ml/dataset.py](file://MTAO/taogen/ml/dataset.py)
...
实现：[ml/model.py](file://MTAO/taogen/ml/model.py)
...
实现：[ml/train.py](file://MTAO/taogen/ml/train.py) +
[scripts/train_run.sh](file://MTAO/taogen/scripts/train_run.sh) /
[scripts/train_smoke.sh](file://MTAO/taogen/scripts/train_smoke.sh)。
...
| 脚本 | 用途 | 时长 |
|---|---|---|
| [train_smoke.sh](file://MTAO/taogen/scripts/train_smoke.sh) | 30 步冒烟，验证 dataset/model/loss/ckpt 链路 | ≤ 60s |
| [train_run.sh](file://MTAO/taogen/scripts/train_run.sh)     | 1 epoch ≈ 1h 的常规训练 | 由 `EPOCHS` 控制 |
...
- ✅ oracle 字段在训练时由探针给出，在推理时由 [mesi_ref_sim](file://MTAO/taogen/mesi_ref_sim) 重放给出；
...
- [tao_trace.cc](file://MTAO/taogen/gem5_patches/src/cpu/o3/probe/tao_trace.cc)
- [TaoTrace.py](file://MTAO/taogen/gem5_patches/src/cpu/o3/probe/TaoTrace.py)
- [run_mt_mvp.py](file://MTAO/taogen/configs/run_mt_mvp.py)
- [single_core_arch_A.json](file://MTAO/taogen/configs/single_core_arch_A.json)
- [step5_run_4workloads.sh](file://MTAO/taogen/configs/step5_run_4workloads.sh)
- [step5_l3_4mib_smoke.sh](file://MTAO/taogen/configs/step5_l3_4mib_smoke.sh)
- [run_experiment.sh](file://MTAO/taogen/scripts/run_experiment.sh)
- 4 个 workloads/[mt_*.c](file://MTAO/taogen/workloads)
- [shared/uarch_profile.hh](file://MTAO/taogen/shared/uarch_profile.hh)
- [shared/lru_banked.hh](file://MTAO/taogen/shared/lru_banked.hh)
- [mesi_ref_sim/src/main.cc](file://MTAO/taogen/mesi_ref_sim/src/main.cc)
- [mesi_ref_sim/include/simulator.hpp](file://MTAO/taogen/mesi_ref_sim/include/simulator.hpp)

数据集生产：

- [tools/derive_mem_events.py](file://MTAO/taogen/tools/derive_mem_events.py)
- [tools/extract_uarch_profile.py](file://MTAO/taogen/tools/extract_uarch_profile.py)
- [tools/build_micro_dataset.py](file://MTAO/taogen/tools/build_micro_dataset.py)
- [tools/check_micro_alignment.py](file://MTAO/taogen/tools/check_micro_alignment.py)
- [tools/sample_steady_balanced.py](file://MTAO/taogen/tools/sample_steady_balanced.py)
- [tools/pack_to_parquet.py](file://MTAO/taogen/tools/pack_to_parquet.py)
- [tools/subsample_dataset.py](file://MTAO/taogen/tools/subsample_dataset.py)
- [scripts/build_3m_dataset.sh](file://MTAO/taogen/scripts/build_3m_dataset.sh)
- [scripts/pack_3m.sh](file://MTAO/taogen/scripts/pack_3m.sh)
...
- [ml/dataset.py](file://MTAO/taogen/ml/dataset.py)
- [ml/model.py](file://MTAO/taogen/ml/model.py)
- [ml/train.py](file://MTAO/taogen/ml/train.py)
- [scripts/train_run.sh](file://MTAO/taogen/scripts/train_run.sh)
- [scripts/train_smoke.sh](file://MTAO/taogen/scripts/train_smoke.sh)
