...
SCONS_BIN="${SCONS_BIN:-$(command -v scons)}"
PYTHON_BINDIR="$(cd "$(dirname "$PYTHON_BIN")" && pwd)"
PYTHON_LIBDIR="$(cd "$PYTHON_BINDIR/.." && pwd)/lib"
if [ -x "$PYTHON_BINDIR/python3-config" ]; then
  PYTHON_CONFIG_BIN="${PYTHON_CONFIG_BIN:-$PYTHON_BINDIR/python3-config}"
elif [ -x "$PYTHON_BINDIR/python-config" ]; then
  PYTHON_CONFIG_BIN="${PYTHON_CONFIG_BIN:-$PYTHON_BINDIR/python-config}"
else
  PYTHON_CONFIG_BIN="${PYTHON_CONFIG_BIN:-$(command -v python3-config || command -v python-config)}"
fi
...
echo "[taogen] SCONS=$SCONS_BIN"
echo "[taogen] PYTHON_CONFIG=$PYTHON_CONFIG_BIN"
echo "[taogen] PYTHON_LIBDIR=$PYTHON_LIBDIR"

# ---------- 0) prerequisites ----------
for tool in git "$SCONS_BIN" cmake make gcc g++ "$PYTHON_BIN" "$PYTHON_CONFIG_BIN"; do
...
( cd "$GEM5_DIR" && \
  ...
  "$PYTHON_BIN" "$SCONS_BIN" defconfig build/X86_MESI_Three_Level "$REPO/gem5_patches/build_opts/X86_MESI_Three_Level" )

echo "[taogen] building gem5.opt X86_MESI_Three_Level (-j$JOBS)"
( cd "$GEM5_DIR" && \
  TAOGEN_SHARED="$SHARED_INC" \
  PYTHON_CONFIG="$PYTHON_CONFIG_BIN" \
  "$PYTHON_BIN" "$SCONS_BIN" build/X86_MESI_Three_Level/gem5.opt PROTOCOL=MESI_Three_Level -j"$JOBS" )
