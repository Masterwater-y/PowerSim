...
最终验证负载：
- [mt_micro_coh](file:///data00/yinhaolang/simulators/single_core_mvp/workloads/mt_micro_coh/mt_micro_coh.c) (4 cores × 5000 iter): 40683/40683 strict-aligned
- [mt_coh_stress](file:///data00/yinhaolang/simulators/single_core_mvp/workloads/mt_coh_stress/mt_coh_stress.c) (4 cores × 1000 iter, 27s gem5): 77567/77567 strict-aligned，混淆矩阵纯对角覆盖 5 类 (L1/LLC/DRAM/L2/WB)
...
[compare_oracle.py](file:///data00/yinhaolang/simulators/single_core_mvp/mesi_ref_sim/scripts/compare_oracle.py) 用 `seq` 作 dict key，但 `seq` 在每个 core 上独立计数 → 4 个 core 之间互相覆盖，3/4 的 ref_sim 预测被错误丢弃。

修复：key 改为 `(seq, core_id)`，oracle 输出和 ref_sim 输出都同步带 `core_id`。

### 真因 2（贡献 ~10%）：vaddr/paddr 共用 LRU 容器

[tao_trace.cc](file:///data00/yinhaolang/simulators/gem5/src/cpu/o3/probe/tao_trace.cc) 内 oracle 的两条路径用了不同地址：packet 路径用 `pkt->getAddr()`(paddr)，fallback 路径用 `inst->effAddr`(vaddr)。两套 entry 混在同一容量受限 LRU 里互相驱逐。
...
修复：[simulator.hpp](file:///data00/yinhaolang/simulators/single_core_mvp/mesi_ref_sim/include/simulator.hpp) 改成 FA-LRU。
...
| 改动点 | 行数 | 文件 |
|---|---|---|
| gem5 emit `request` 事件 | +18 | [tao_trace.cc](file:///data00/yinhaolang/simulators/gem5/src/cpu/o3/probe/tao_trace.cc) `onMemDataAccessComplete` |
| ref_sim 处理 `request` 事件 | +14 | [main.cc](file:///data00/yinhaolang/simulators/single_core_mvp/mesi_ref_sim/src/main.cc) |
| compare 改 request 轴评估 | +25 | [compare_oracle.py](file:///data00/yinhaolang/simulators/single_core_mvp/mesi_ref_sim/scripts/compare_oracle.py) |
...
[Sequencer.cc#L817-L825](file:///data00/yinhaolang/simulators/gem5/src/mem/ruby/system/Sequencer.cc#L817-L825) 加了：
...
[MESI_Three_Level-L0cache.sm](file:///data00/yinhaolang/simulators/gem5/src/mem/ruby/protocol/MESI_Three_Level-L0cache.sm) / [-L1cache.sm](file:///data00/yinhaolang/simulators/gem5/src/mem/ruby/protocol/MESI_Three_Level-L1cache.sm) 在所有 `sendResponse` 类 action 中**从未设置 `out_msg.Sender := machineID`** —— SLICC 协议层不告知"响应来自哪台机器"，hitCallback 收到的 `mach` 永远是 `MachineType_NUM`。
...
[Sequencer.cc#L817-L825](file:///data00/yinhaolang/simulators/gem5/src/mem/ruby/system/Sequencer.cc#L817-L825) 的桥接代码保留 —— 当前 mach=NUM 永不进分支，**0 副作用**；一旦未来 .sm 改造完成，逻辑**自动激活**。
...
- oracle emit request: [tao_trace.cc onMemDataAccessComplete](file:///data00/yinhaolang/simulators/gem5/src/cpu/o3/probe/tao_trace.cc#L1213)
- evict hook: [CacheMemory.cc deallocate](file:///data00/yinhaolang/simulators/gem5/src/mem/ruby/structures/CacheMemory.cc)
- prefetch hook: [RubyPrefetcherProxy.cc notifyPfFill](file:///data00/yinhaolang/simulators/gem5/src/mem/ruby/structures/RubyPrefetcherProxy.cc)
- ref_sim 透传 packet 真值: [main.cc#L86](file:///data00/yinhaolang/simulators/single_core_mvp/mesi_ref_sim/src/main.cc#L86)
- 对齐验证负载: [mt_coh_stress.c](file:///data00/yinhaolang/simulators/single_core_mvp/workloads/mt_coh_stress/mt_coh_stress.c)（covers L1/L2/LLC/DRAM/WB/migration/false-sharing）
...
在 V5 100% 标签对齐基础上派生 PMU 计数：oracle 端和 ref_sim 端用同一聚合规则，互为 bit-exact 校验；对 [pmu_events.txt](file:///data00/yinhaolang/simulators/uncore_msr/pmu_events.txt) L18/21/22/23 + cache miss 子集做覆盖。脚本：[pmu_report.py](file:///data00/yinhaolang/simulators/single_core_mvp/mesi_ref_sim/scripts/pmu_report.py)。
...
V7 归档：[backups/v7_20260522/](file:///data00/yinhaolang/simulators/backups/v7_20260522/)（含 pmu_report.py / 完整 trace / stats.txt / v7_pmu_report.txt）。
...
### 问题
V7 的 `cache.llc.store_misses` 与 `TOR_INSERTS.IA_MISS_DRD`（store 部分）= 0：V5 strict 路径只在 load packet response 上打 oracle_source=0 真值；store retire 走 fallback (oracle_source=1, 基于 [tao_trace.cc](file:///data00/yinhaolang/simulators/gem5/src/cpu/o3/probe/tao_trace.cc) 的 `line_states_` 投影)。

预诊：fallback 路径 store-side `coh==DRAM` = 4031，对比 ruby `L2Cache_Controller.IM.Mem_Data::total = 3960`，理论可达 98.21% 准确度。

### 改动
[pmu_report.py](file:///data00/yinhaolang/simulators/single_core_mvp/mesi_ref_sim/scripts/pmu_report.py#L201-L237) 移除 `oracle_source==0` 限制，PMU 聚合放宽到全部 `request` 行；strict-eval（[compare_oracle.py](file:///data00/yinhaolang/simulators/single_core_mvp/mesi_ref_sim/scripts/compare_oracle.py)）保持只用 oracle_source==0 不变。oracle 与 ref_sim 用同一聚合规则 → 互相 bit-exact 仍保证。

[parse_stats](file:///data00/yinhaolang/simulators/single_core_mvp/mesi_ref_sim/scripts/pmu_report.py#L248-L287) 扩展解析 `L2Cache_Controller.{ISS,IS,IM}.Mem_Data::total`，输出 `llc_load_miss / llc_store_miss / llc_miss_total` 作为 ruby 真值列。
...
V8 归档：[backups/v8_20260524/](file:///data00/yinhaolang/simulators/backups/v8_20260524/)（含 pmu_report.py / 完整 trace / stats.txt / v8_pmu_report.txt）。
...
### Detailed 跑路径
- 现状：[run_mt_mvp.py](file:///data00/yinhaolang/simulators/single_core_mvp/configs/run_mt_mvp.py) +
  [tao_trace.cc](file:///data00/yinhaolang/simulators/gem5/src/cpu/o3/probe/tao_trace.cc) 5 jsonl 输出已稳定
...
1. ✅ **归档 V9 路线决策**（本节）
2. ✅ **新增 atomic 跑路径**：[run_mt_mvp_atomic.py](file:///data00/yinhaolang/simulators/single_core_mvp/configs/run_mt_mvp_atomic.py)（X86/gem5.opt + AtomicSimpleCPU + NoCache + classic DDR4）
3. ✅ **修复 W1/W7 detailed+Ruby BSS fatal**：开 RubySystem.access_backing_store=True + 挂 phys_mem(SimpleMemory, in_addr_map=False)（参见 [run_mt_mvp.py](file:///data00/yinhaolang/simulators/single_core_mvp/configs/run_mt_mvp.py#L97-L117)）
4. ✅ **W1/W7 最小验证数据采集**：
...
**推翻 V1-V8 macro 路径**：之前 [tao_trace.cc](file:///data00/yinhaolang/simulators/gem5/src/cpu/o3/probe/tao_trace.cc) 用 `accumulateMicro + flushMacro` 输出 macro 粒度是工程妥协（一条 macro 含 N 条 micro 时把 latency/MESI/sharer 折叠/取首条），不符合理论需求。已知 ref_simulator 100% 对齐结论保留为 macro baseline 备份（[backups/v9_macro_baseline_20260525/](file:///data00/yinhaolang/simulators/backups/v9_macro_baseline_20260525/)）。
...
**实现要点**：
- [TaoTrace.py](file:///data00/yinhaolang/simulators/gem5/src/cpu/o3/probe/TaoTrace.py) 新增 `emit_macro: Bool=False` / `emit_micro: Bool=True` 参数
- [tao_trace.cc](file:///data00/yinhaolang/simulators/gem5/src/cpu/o3/probe/tao_trace.cc)：
...
- atomic 侧 [atomic_func_trace.{hh,cc}](file:///data00/yinhaolang/simulators/gem5/src/cpu/simple/probes/atomic_func_trace.hh) 重写为 micro 粒度：每条 commit 落盘，producer-distance 在 micro 粒度
...
**结果**（[check_micro_alignment.py](file:///data00/yinhaolang/simulators/single_core_mvp/tools/check_micro_alignment.py)）：
...
- [atomic_func_trace.hh](file:///data00/yinhaolang/simulators/gem5/src/cpu/simple/probes/atomic_func_trace.hh)：micro 粒度 functional probe 头文件
- [atomic_func_trace.cc](file:///data00/yinhaolang/simulators/gem5/src/cpu/simple/probes/atomic_func_trace.cc)：每 commit 一行，producer-distance K=4
- [tao_trace.cc](file:///data00/yinhaolang/simulators/gem5/src/cpu/o3/probe/tao_trace.cc) `emitMicroRecord`：detailed 侧 micro emit
- [TaoTrace.py](file:///data00/yinhaolang/simulators/gem5/src/cpu/o3/probe/TaoTrace.py)：emit_micro / emit_macro 开关
- [check_micro_alignment.py](file:///data00/yinhaolang/simulators/single_core_mvp/tools/check_micro_alignment.py)：对齐校验脚本
- [backups/v9_macro_baseline_20260525/](file:///data00/yinhaolang/simulators/backups/v9_macro_baseline_20260525/)：V1-V8 macro baseline 冻结
...
- [build_micro_dataset.py](file:///data00/yinhaolang/simulators/single_core_mvp/tools/build_micro_dataset.py)：差分公式 + sum 自检 + 负值统计
- [datasets/v9_micro/](file:///data00/yinhaolang/simulators/datasets/v9_micro/)：W1+W7 训练样本
...
- [build_micro_dataset.py](file:///data00/yinhaolang/simulators/single_core_mvp/tools/build_micro_dataset.py)：端到端公式 + max 累加自检
- [datasets/v9_micro/](file:///data00/yinhaolang/simulators/datasets/v9_micro/)：W1+W7 训练样本（重新生成）
...
1. **[lsq_unit.cc:1100-1117](file:///data00/yinhaolang/simulators/gem5/src/cpu/o3/lsq_unit.cc#L1100-L1117)**
   `LSQUnit::writeback()` 在 cache 数据返回时**覆盖** `inst->completeTick = curTick() - inst->fetchTick`。
   原因：[iew.cc:1559](file:///data00/yinhaolang/simulators/gem5/src/cpu/o3/iew.cc#L1559) `updateExeInstStats` 在 IEW execute 阶段就提前设置 completeTick，
   对 cache miss load 完全失真（V9.3 W7 旧值始终 < 1k tick，与真实 DRAM stall ~数百 cycle 不符）。
2. **[tao_trace.cc:1374-1392](file:///data00/yinhaolang/simulators/gem5/src/cpu/o3/probe/tao_trace.cc#L1374-L1392)**
...
[build_micro_dataset.py](file:///data00/yinhaolang/simulators/single_core_mvp/tools/build_micro_dataset.py) 改用：
...
- [lsq_unit.cc#L1100-L1117](file:///data00/yinhaolang/simulators/gem5/src/cpu/o3/lsq_unit.cc#L1100-L1117)：completeTick 覆盖
- [tao_trace.cc#L1374-L1392](file:///data00/yinhaolang/simulators/gem5/src/cpu/o3/probe/tao_trace.cc#L1374-L1392)：ready_tick 字段
- [build_micro_dataset.py](file:///data00/yinhaolang/simulators/single_core_mvp/tools/build_micro_dataset.py)：V9.4 公式
- [datasets/v9_micro/](file:///data00/yinhaolang/simulators/datasets/v9_micro/)：W1+W7 V9.4 训练样本（重新生成）
...
通读 [atomic_func_trace.cc](file:///data00/yinhaolang/simulators/gem5/src/cpu/simple/probes/atomic_func_trace.cc)
与 [tao_trace.cc#L1324-L1372](file:///data00/yinhaolang/simulators/gem5/src/cpu/o3/probe/tao_trace.cc#L1324-L1372)
...
2. **build_micro_dataset.py 改造**（[build_micro_dataset.py](file:///data00/yinhaolang/simulators/single_core_mvp/tools/build_micro_dataset.py)）：
...
- [build_micro_dataset.py](file:///data00/yinhaolang/simulators/single_core_mvp/tools/build_micro_dataset.py)：V9.5 单源 builder
- [datasets/v9_micro/](file:///data00/yinhaolang/simulators/datasets/v9_micro/)：W1+W7 V9.5 训练样本（重新生成，total=4302155）
- 双跑遗产保留作部署/推理输入：[atomic_func_trace.cc](file:///data00/yinhaolang/simulators/gem5/src/cpu/simple/probes/atomic_func_trace.cc)
