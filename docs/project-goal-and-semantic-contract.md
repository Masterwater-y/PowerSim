...
状态：规范性（normative）
生效日期：2026-08-18
修订日期：2026-09-11（多核采集与指标统一使用共同终点）
...
### 3.0 多核采集与计分必须在共同终点关闭

**规则自 2026-09-11 起生效，优先于历史逐核截断和“固定逐核分母＋不计分后缀”方案。**
规约身份为 `first-core-target-common-end-v1`；这是要求实现的边界合同标识，不表示
当前采集器、oracle 或 validator 已经支持该字段或通过新门禁。

设参与本次测量的核心集合为 `C`，目标为最快核达到 `N = 10,000,000` 个用户 UOP。
`U_i(t)` 为核心 `i` 在已声明测量窗口和用户 UOP 口径下的累计计数。停止条件为：

```text
first_target_reached(t) = 存在 i ∈ C，使 U_i(t) >= N
```

**最快核首次达标时，所有参与核心在同一个事件结束采集和指标统计，不再等待最慢核。**
这替代此前同日提出的“等所有核达标”规则。沿用目标触发核的完整宏指令边界判定，
因此触发核可以有完成当前宏指令所必需的少量 UOP 超额；不得另加任意记录尾段。
其他核允许不足 `N`，其共同窗口内实际记录的全部工作都计分。

例如，目标为最快核 10M 用户 UOP，共同终点处 A 核执行 10M、B 核执行 6M，则：

| 核心 | 正式计分的用户 UOP | 采集与指标结束条件 |
|---|---:|---|
| A | 10M | A 核达标触发共同终点 |
| B | 6M | 同一个共同终点，不等待 B 到 10M |
| 合计 | 16M | 不得用 2 × 10M 替代实际人口 |

必须同时满足以下合同：

1. **FST 连续性。** 共同终点前持续输出所有按既有 scope 应记录的真实 UOP；完整 RAW、
   地址空间、映射、syscall 元数据和 CPL 状态识别同步继续。仅在首核达标的共同事件关闭
   采集；不得为慢核另外延长或补齐记录。内核与 idle 的纳入规则保持所选 scope 的既有定义，不能为凑
   目标混用宏指令、用户 UOP 和混合记录数。
2. **CPI 分子与分母一起继续。** gem5 的周期分类、退休 UOP、退休宏指令等统计持续到
   同一个共同终点。FastSim 使用对应的完整功能范围自行推理周期，并从实际退休记录
   计算分母；慢核不能补齐到 `N`，也不能继续累计共同事件之后的周期。每核 CPI 为其 scope 匹配的
   周期 / 实际宏指令数；整机 CPI 为累计周期 / 累计实际宏指令数，不是逐核 CPI 的平均。
3. **PMU 一起继续。** 分支、缓存、TLB、一致性、DRAM 等计数按各自事件字典累计同一
   测量范围，在首核达标的同一个事件关闭所有核的 oracle。对于按测量内 committed 请求归属的生命周期
   PMU，共同终点后仍可结清已有请求，但不得将 drain 期间的新工作混入原人口或把 drain
   时长直接加进 CPI；按发生时间归属的事件仍按字典截取共同窗口。
4. **吞吐按实际工作量。** 分子采用共同终点内实际处理的用户 UOP 数，不能写死为
   `核心数 × N`；预热单独报告并排除在测量段吞吐之外。共同终点前的全部 UOP
   同时属于实际处理工作和正式计分工作，包含不足 `N` 的慢核。
5. **可审计的共同边界。** 保存参与核心集合、目标值及单位、共同起止事件身份、停止
   原因、每核实际用户/内核 UOP、宏指令与排他记录边界。同一 tick 的提交顺序、边界处
   微码/宏指令及辅助记录的归属必须一致定义。非触发核可以恰好在宏指令中途或 syscall
   入口结束：保留此前真实 UOP，仅统计已完成宏指令，不补记录、不伪造末 UOP，
   FST 和 oracle 不得各自取整到不同窗口。
   gem5 tick 仅作离线边界证明，不得作为 FastSim 的时序答案输入。
6. **完整性与失败。** 正常完成时触发核实际用户 UOP 必须 `>= N`，慢核不足 `N`
   是合法结果。所有预先声明的核心必须保留，且共同结束事件、FST/oracle/replay 人口
   必须一致。没有核心达标就提前 EOF、被中止或达到保护上限，不能作为完成样本。
   不得事后删除慢核；结清在飞请求的结束原因另行记录。

因此，默认正式测量的 **采集结束边界、计分结束边界和指标窗口对应同一个共同事件**。
原 `fastsim-binary-context-v1` 支持的“某核先冻结计分、再执行不计分后缀”可保留为
明确标注的历史对照/诊断能力，但不能替代本条规定。仅修改 manifest、令
`score_end == execution_end`，不能补齐原采集器已经丢弃的 UOP 或证明采集曾在共同
终点关闭。

迁移必须同时修改采集器、CPI/PMU oracle、边界元数据、转换/manifest 生成、FastSim
计分和校验/汇总工具，并用不等速多核 case 验证最快核达标、慢核不足 `N` 时，
所有核的记录与指标仍在同一个事件正确关闭。旧逐核 10M 数据、原 formal40 报告及固定分母后缀实验保留为历史证据；未按
新共同窗口重新采集并生成匹配参考前，不得直接标记为符合本规约的正式精度结果。
