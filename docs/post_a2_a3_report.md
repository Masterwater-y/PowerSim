...
> 测试入口：[step5_run_4workloads.sh](file:///data00/yinhaolang/simulators/single_core_mvp/configs/step5_run_4workloads.sh)
...
| 模块 | 文件 | 改动 |
|---|---|---|
| 共享头 | [uarch_profile.hh](file:///data00/yinhaolang/simulators/single_core_mvp/shared/uarch_profile.hh) | schema v2 + 自实现 JSON parser；fail-fast on 不支持微架构 |
| 共享头 | [lru_banked.hh](file:///data00/yinhaolang/simulators/single_core_mvp/shared/lru_banked.hh) | 4-bank × N-way LRU + TlbSim + MshrTracker + PageWalkSim |
| oracle 探针 | [tao_trace.hh](file:///data00/yinhaolang/simulators/gem5/src/cpu/o3/probe/tao_trace.hh) / [tao_trace.cc](file:///data00/yinhaolang/simulators/gem5/src/cpu/o3/probe/tao_trace.cc) | A2: i-cache probe (ppInstAccessComplete 监听) + d-side MSHR；A3: per-core L1D/L1I/L2 + 共享 L3 banked LRU + iTLB/dTLB + 4 级 page walker |
| oracle 探针 | [TaoTrace.py](file:///data00/yinhaolang/simulators/gem5/src/cpu/o3/probe/TaoTrace.py) | 新增 `uarch_profile_path` 参数；`emit_mem_events` 与 `emit_macro` 解耦（B 方案） |
| ref_sim | [simulator.hpp](file:///data00/yinhaolang/simulators/single_core_mvp/mesi_ref_sim/include/simulator.hpp) / [main.cc](file:///data00/yinhaolang/simulators/single_core_mvp/mesi_ref_sim/src/main.cc) | 切换到 UarchProfile + 共享 LRU/TLB/MSHR/Walker；新增 `stepIFetch()` 与 `cache_level==4` (i-cache) |
| 流程 | [run_mt_mvp.py](file:///data00/yinhaolang/simulators/single_core_mvp/configs/run_mt_mvp.py) | 9 个新 CLI 参数（dtlb/itlb/mshr/page-size/walker-levels）+ 仿真前 `write_uarch_profile()` |
| 校验 | [compare_oracle.py](file:///data00/yinhaolang/simulators/single_core_mvp/mesi_ref_sim/scripts/compare_oracle.py) | 跳过 ifetch 行（专为 d-side） |
| 校验 | [compare_ifetch.py](file:///data00/yinhaolang/simulators/single_core_mvp/mesi_ref_sim/scripts/compare_ifetch.py) | **新增**：i-side 4 字段 bit-exact |
| 校验 | [pmu_report.py](file:///data00/yinhaolang/simulators/single_core_mvp/mesi_ref_sim/scripts/pmu_report.py) | `--uarch-profile` 取代 hardcode 64B line |
...
baseline 报告（[diagnosis/baseline_pre_a2_a3.md](file:///data00/yinhaolang/simulators/diagnosis/baseline_pre_a2_a3.md)）记录了 A2/A3 前的 fallback% / oracle LRU 抖动等病灶。本轮修复后：
