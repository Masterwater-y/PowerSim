/*
 * v28 business-oriented functional workloads for FastSim validation.
 *
...
static void try_pin_cpu(int tid)
{
    cpu_set_t set;
    int cpu_base = 0;
    const char *base = getenv("FASTSIM_CPU_BASE");
    if (base && *base) cpu_base = atoi(base);
    CPU_ZERO(&set);
    CPU_SET(cpu_base + tid, &set);
    if (pthread_setaffinity_np(pthread_self(), sizeof(set), &set) != 0) {
        fprintf(stderr, "%s: pthread_setaffinity_np(tid=%d) failed: %s\n",
                V28_NAME, cpu_base + tid, strerror(errno));
        abort();
    }
}
...
static void *worker_main(void *argp)
{
    ...
    fastsim_roi_measurement_begin((uint64_t)arg->tid);
