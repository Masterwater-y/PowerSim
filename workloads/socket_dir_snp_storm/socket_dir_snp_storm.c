...
static void *worker_main(void *arg) {
  ...
  for (uint64_t pass = 0; pass < shared->passes; pass++) {
    /*
     * Phase A: rotate ownership of every cache line across threads.
     * A line written by thread t in pass p will be written by thread t+1 in the
     * next pass, forcing repeated ownership transfer and directory snoop lookup.
     */
    for (size_t line = (size_t)ctx->tid; line < shared->line_count; line += (size_t)shared->threads) {
      if (((line + (size_t)pass) % (size_t)shared->threads) != (size_t)ctx->tid) {
        continue;
      }
      uint64_t *base = shared->words + line * words_per_line;
      uint64_t seed = mix64(((uint64_t)line << 17) ^ (pass * 0x100000001b3ULL) ^ checksum);
      for (size_t w = 0; w < words_per_line; w++) {
        uint64_t value = base[w];
        value ^= seed + (uint64_t)w * 0x94d049bb133111ebULL;
        value = rotl64(value, (unsigned)((ctx->tid + w + pass) % 29 + 3));
    ...
    /*
     * Phase B: every thread reads the same shared lines in a different order,
     * rebuilding a multi-sharer state before the next ownership rotation.
     */
    for (size_t line = (size_t)ctx->tid; line < shared->line_count; line += (size_t)shared->threads) {
      size_t idx = (line * 1315423911u + (size_t)ctx->tid * 17u) % shared->line_count;
      uint64_t *base = shared->words + idx * words_per_line;
      uint64_t local = checksum ^ ((uint64_t)idx << 9) ^ pass;
      for (size_t w = 0; w < words_per_line; w++) {
        local ^= mix64(base[w] + (uint64_t)w * 0x9e3779b97f4a7c15ULL);
      }
      checksum = rotl64(checksum ^ local, 7) + 0xbf58476d1ce4e5b9ULL;
    }
...
int main(int argc, char **argv) {
  ...
  uint64_t shared_mb = (argc >= 4) ? parse_u64(argv[3]) : 64;
  ...
  printf("[socket_dir_snp_storm] passes=%" PRIu64 " threads=%d shared_mb=%" PRIu64 " shared_bytes=%zu\n",
         passes, threads, shared_mb, shared_bytes);
  printf("[socket_dir_snp_storm] allowed_cpus=");
