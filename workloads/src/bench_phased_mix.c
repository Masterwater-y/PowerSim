/* bench_phased_mix — 相位混合：每个 worker 在一个外层循环里轮换执行
 *   [ALU 计算相 -> 私有随机读访存相 -> 难预测分支相]，制造真实程序的
 *   相位过渡与 PMU 联合分布（microbench 缺失的关键）。
 * 修正：
 *   1) 访存相改成私有只读主导，避免原实现随机写回在 gem5 Ruby 下触发 panic。
...
static void kernel(int tid, int nthreads, long scale, void *shared)
{
    ...
    long rounds = scale * 8;
    /* 每线程私有访存 buffer（512KiB，跨 L2；私有只读主导，稳定性更好） */
    size_t bn = 64 * 1024;
    ...
    for (long rd = 0; rd < rounds; rd++) {
        ...
        /* 相 2：私有随机读访存（高 LLC miss、高 CPI，但避免随机写回） */
        size_t p = acc % bn;
...
TAO_BENCH_MAIN("phased_mix", kernel, NULL)
