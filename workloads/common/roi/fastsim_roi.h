...
static inline void
fastsim_roi_measurement_begin(uint64_t thread_id)
{
    register uint64_t work_id_reg __asm__("rdi") = 0;
    register uint64_t thread_id_reg __asm__("rsi") = thread_id;
#ifdef FASTSIM_QEMU_FST_TRACE
...
#else
    __asm__ __volatile__(
        "nop; .byte 0x0F, 0x04; .word 0x005c"
        : "+D"(work_id_reg), "+S"(thread_id_reg)
        :
        : "rax", "rcx", "rdx", "r8", "r9", "r10", "r11", "cc", "memory");
#endif
...
{
#if defined(FASTSIM_QEMU_FST_TRACE)
