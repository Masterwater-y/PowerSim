...
RUN_NAME=${RUN_NAME:-tcsim_v30_gss_p1_frozen_adapter_5k_seed1234_watch}
OUT=${OUT:-ckpt/tcsim_v30_gss_p1_frozen_adapter_5k_seed1234}
MANIFEST=${MANIFEST:-data/v30_gss_ready_dataset/manifest.json}
CONFIG=${CONFIG:-configs/v30_gss_p1_frozen_adapter.yaml}
...
for required in "$MANIFEST" "$CONFIG" "$V29_CHECKPOINT"; do
  [[ -f "$required" ]] || {
    echo "[v30-gss-p1][ERROR] missing required file: $required" >&2
    exit 2
  }
done
if ! command -v nvidia-smi >/dev/null 2>&1 || ! nvidia-smi -L >/dev/null 2>&1; then
  echo "[v30-gss-p1][ERROR] NVIDIA driver/GPU is not visible" >&2
  exit 3
fi
IFS=',' read -r -a GPU_ITEMS <<< "$GPU_LIST"
if (( ${#GPU_ITEMS[@]} != NPROC_VALUE )); then
  echo "[v30-gss-p1][ERROR] GPUS count != NPROC: $GPU_LIST vs $NPROC_VALUE" >&2
  exit 3
fi

# Put the watchdog in its own session as well as disconnecting stdio.  This is
# required when the launcher itself is executed by a supervised command shell.
nohup setsid env \
  ROOT="$ROOT" \
  RUN_NAME="$RUN_NAME" \
  TRAIN_SCRIPT=scripts/run_v30_gss_p1_ddp8.sh \
  MANIFEST="$MANIFEST" \
  CONFIG="$CONFIG" \
