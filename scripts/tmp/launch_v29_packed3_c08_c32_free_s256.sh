...
RUN_TAG=v29_packed3_free_s${TARGET_STRIDE}_seed0_c08_c32_${RUN_STAMP}
...
[[ -x "$PY" ]] || { echo "[v29-free-s256][ERROR] missing Python: $PY" >&2; exit 2; }
[[ -f "$CKPT" ]] || { echo "[v29-free-s256][ERROR] missing checkpoint: $CKPT" >&2; exit 2; }
[[ -f "$MANIFEST" ]] || { echo "[v29-free-s256][ERROR] missing manifest: $MANIFEST" >&2; exit 2; }
...
printf '[v29-free-s256] pid=%s\n' "$launcher_pid"
printf '[v29-free-s256] mode=free target_stride=%s max_step_cycles=%s\n' "$TARGET_STRIDE" "$MAX_STEP_CYCLES"
printf '[v29-free-s256] optimization=numpy-vectorized-context cpu-window-cache free-fast-path horizon-outputs-off oracle-drift-off\n'
printf '[v29-free-s256] out=%s\n' "$OUT_DIR"
printf '[v29-free-s256] log=%s\n' "$LAUNCH_LOG"
printf '[v29-free-s256] monitor: tail -f %q\n' "$LAUNCH_LOG"
