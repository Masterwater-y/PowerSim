...
RUN_NAME=${RUN_NAME:-tcsim_v29_frozen_memory_probe_e2_10k_seed1234_watch}
OUT=${OUT:-ckpt/tcsim_v29_frozen_memory_probe_e2_10k_seed1234}
MANIFEST=${MANIFEST:-data/v29_long_history_dataset/manifest.json}
CONFIG=${CONFIG:-configs/v29_frozen_memory_probe.yaml}
E0_CHECKPOINT=${E0_CHECKPOINT:-ckpt/tcsim_v29_packed3_100m_8gpu_60k/best.pt}
GPU_LIST=${GPUS:-0,1,2,3,4,5,6,7}
NPROC_VALUE=${NPROC:-8}

for required in "$MANIFEST" "$CONFIG" "$E0_CHECKPOINT"; do
  [[ -f "$required" ]] || {
    echo "[v29-frozen-probe][ERROR] missing required file: $required" >&2
    exit 2
  }
done
if ! command -v nvidia-smi >/dev/null 2>&1 || ! nvidia-smi -L >/dev/null 2>&1; then
  echo "[v29-frozen-probe][ERROR] NVIDIA driver/GPU is not visible; training was not started" >&2
  exit 3
fi
IFS=',' read -r -a GPU_ITEMS <<< "$GPU_LIST"
if (( ${#GPU_ITEMS[@]} != NPROC_VALUE )); then
  echo "[v29-frozen-probe][ERROR] GPUS count != NPROC: $GPU_LIST vs $NPROC_VALUE" >&2
  exit 3
fi
...
nohup env \
  ROOT="$ROOT" \
  RUN_NAME="$RUN_NAME" \
  TRAIN_SCRIPT=scripts/run_v29_frozen_memory_probe_ddp8.sh \
  MANIFEST="$MANIFEST" \
  CONFIG="$CONFIG" \
  E0_CHECKPOINT="$E0_CHECKPOINT" \
  OUT="$OUT" \
