...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Emit records.micro only; disable labels.micro and mem_events.",
    )
    ...
    if not args.no_tao_trace:
        for i, core in enumerate(tao_target_cores):
            core.core.tao_trace = TaoTrace(
                ...
                emit_labels=not args.records_only,
                emit_mem_events=not args.records_only,
                functional_records_only=args.records_only,
            )
