...
TaoTrace::TaoTrace(const TaoTraceParams &params)
    : ProbeListenerObject(params),
      ...
      emit_labels_(params.emit_labels),
      functional_records_only_(params.functional_records_only),
                          ...
                          uint16_t branch_history)
{
    ...
    for (const auto &destination : dsts) {
        const auto cls = destination.first;
        fatal_if(cls >= 4 || destination_class_counts[cls] == 31,
                 "destination register class count exceeds FST v6 width");
        ++destination_class_counts[cls];
    }
    ...
    if (out_records_micro_) {
        ...
        if (!functional_records_only_) {
            std::fprintf(out_records_micro_,
            ",\"thread_id\":%u,\"seq_num\":%" PRIu64 ","
            "\"cacheline_paddr\":%" PRIu64 ","
