...
class TaoTrace(ProbeListenerObject):
    ...
    emit_labels = Param.Bool(True,
        "Emit per-micro labels.micro.jsonl when emit_micro is enabled.")
    functional_records_only = Param.Bool(False,
        "Emit only the raw functional record fields required by external "
        "functional trace consumers.")
