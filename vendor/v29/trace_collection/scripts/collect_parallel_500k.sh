...
GEM5_ROOT=${GEM5_ROOT:-"$(cd "$PROJECT_ROOT/.." && pwd)/gem5"}
GEM5=${GEM5:-"$GEM5_ROOT/build/X86_MESI_Three_Level/gem5.opt"}
CFG=${CFG:-"$PROJECT_ROOT/vendor/v29/gem5_patch/config/run_mt_mvp.py"}
...
PYTHON_LIBDIR=$("$PY" -c 'import sysconfig; print(sysconfig.get_config_var("LIBDIR") or "")')
PYTHON_BASE=$("$PY" -c 'import sys; print(sys.base_prefix)')
if [[ -n "$PYTHON_LIBDIR" && -d "$PYTHON_LIBDIR" ]]; then
  export LD_LIBRARY_PATH="$PYTHON_LIBDIR:${LD_LIBRARY_PATH:-}"
fi
if [[ -d /opt/gcc-11.5.0/lib64 ]]; then
  export LD_LIBRARY_PATH="/opt/gcc-11.5.0/lib64:${LD_LIBRARY_PATH:-}"
fi
...
run_gem5() {
  ...
  set +e
  PYTHONHOME="$PYTHON_BASE" "$GEM5" --outdir="$out_dir" "$CFG" \
    --cmd "$bin" \
...
validate_windows() {
  log "[sanity][error] legacy window validation is not part of v29"
  log "[sanity][error] run scripts/audit_v28_raw_dataset.py and scripts/build_v29_dataset.py instead"
  return 2
}
