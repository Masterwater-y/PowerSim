...
from model.local_model_paths import local_model_path  # noqa: E402
...
PROMPT_SCHEMA_VERSION = "macro-static-bb-prefix-system-v2"
SYSTEM_PROMPT = (
    "You are an x86-64 static instruction semantic encoder. "
    "Encode the target instruction using only normalized static assembly and "
    "same-basic-block predecessor context. Ignore runtime timing, program "
    "identity, absolute addresses, and dynamic execution frequency."
)
POOLING_POLICY = "causal-final-summary-position-v1"
ANCHOR_POLICY = "mean_normalized_target_input_embedding-v1"
CONTEXT_POLICY = "same-bb-previous-4-plus-target-with-fixed-system-v2"
...
def adapter_directory_fingerprint(path: str | Path) -> str:
    """Hash detachable PEFT artifacts without recursively hashing metadata."""

    root = Path(path).resolve()
    if not root.is_dir():
        raise MacroContractError(f"LoRA adapter directory not found: {root}")
    ignored = {"stage_a_manifest.json", "stage_a_manifest.json.tmp"}
    rows = []
    for item in sorted(
        value for value in root.rglob("*")
        if value.is_file() and value.name not in ignored
    ):
        rows.append({
    ...
    if not rows:
        raise MacroContractError(f"LoRA adapter directory is empty: {root}")
    return json_fingerprint(rows)
...
def render_semantic_prompt(
    context: Sequence[str], branch_class: str,
) -> str:
    """Render the shared Stage-A/cache encoder prompt contract.

    This is intentionally a short raw-text task prefix instead of a chat
    template.  Qwen is used as a feature encoder, and training/cache creation
    must feed byte-for-byte equivalent prompt scaffolding.
    """

    if not context:
        raise MacroContractError("semantic prompt context cannot be empty")
    return (
        "System instruction:\n"
        + SYSTEM_PROMPT
        + "\nArchitecture: x86-64\n"
...
def render_semantic_prompt_for_schema(
    context: Sequence[str],
    branch_class: str,
    *,
    prompt_schema_version: str,
) -> str:
    """Render an explicitly versioned cache prompt without silent migration."""

    schema = str(prompt_schema_version)
    if schema == "macro-static-bb-prefix-v1":
        if not context:
            raise MacroContractError("semantic prompt context cannot be empty")
        return (
            "Architecture: x86-64\n"
    ...
    if schema == PROMPT_SCHEMA_VERSION:
        return render_semantic_prompt(context, branch_class)
    raise MacroContractError(
        f"unsupported semantic prompt schema {prompt_schema_version!r}"
    )


def build_static_prompt_records(
    ...
    *,
    ...
    """Build only the PC/prompt table used by online Gradient Cache.

    The prompt schema is taken from the immutable frozen-cache manifest.  This
    keeps a zero-initialized LoRA branch byte-for-byte aligned with B2-frozen
    instead of accidentally adopting whichever prompt version is newest in
    this source file.
    """
    ...
    for key, block in blocks.items():
        block.sort()
        positions[key] = {pc: index for index, pc in enumerate(block)}
    records: List[Dict[str, Any]] = []
    for pc in pcs:
        row = resolver.rows[pc]
        key = (str(row["section_name"]), int(row["bb_id"]))
        block = blocks[key]
        position = positions[key][pc]
        context_pcs = block[
            max(0, position - int(previous_instructions)):position + 1
        ]
        context = [
        ...
        records.append({
            "pc": int(pc),
            "prompt": render_semantic_prompt_for_schema(
                context,
                branch_class,
                prompt_schema_version=prompt_schema_version,
            ),
            "anchor_text": context[-1] + "\n",
        })
    return records
...
def main() -> int:
    ...
    parser.add_argument(
        "--base-model",
        default=local_model_path("Qwen/Qwen2.5-Coder-1.5B-Instruct"),
    )
    parser.add_argument(
        "--lora-adapter",
        default=None,
        help=(
            "optional detachable Stage-A PEFT adapter; it is loaded only for "
            "offline cache generation and is never merged into the base model"
        ),
    )
    ...
    args.base_model = local_model_path(args.base_model)
    if args.lora_adapter is not None:
        args.lora_adapter = str(Path(args.lora_adapter).resolve())
    ...
    if args.lora_adapter is not None:
        adapter_root = Path(args.lora_adapter)
        adapter_config = adapter_root / "adapter_config.json"
        stage_manifest = adapter_root / "stage_a_manifest.json"
        if not adapter_config.is_file() or not stage_manifest.is_file():
            raise MacroContractError(
                "Stage-A LoRA adapter requires adapter_config.json and "
                "stage_a_manifest.json"
            )
        adapter_metadata = json.loads(stage_manifest.read_text())
        if adapter_metadata.get("adapter_schema") != (
            "qwen-macro-stage-a-detachable-lora-v1"
        ):
            raise MacroContractError("unsupported Stage-A LoRA adapter schema")
        if adapter_metadata.get("base_model_config_fingerprint") != config_fp:
            raise MacroContractError("LoRA adapter/base model config mismatch")
        if adapter_metadata.get("base_model_artifact_fingerprint") != artifact_fp:
            raise MacroContractError("LoRA adapter/base model weights mismatch")
        if adapter_metadata.get("dataset_prompt_schema") != PROMPT_SCHEMA_VERSION:
            raise MacroContractError(
                "Stage-A LoRA/cache prompt schema mismatch"
            )
        trained_context_lengths = {
            int(value)
            for value in (adapter_metadata.get("dataset_context_lengths") or [])
        }
        if 4 not in trained_context_lengths:
            raise MacroContractError(
                "Stage-A LoRA was not trained with the cache context length 4"
            )
        lora_contract = dict(adapter_metadata.get("lora", {}) or {})
        if lora_contract.get("merged_into_base") is not False:
            raise MacroContractError("LoRA adapter must remain detachable/unmerged")
        adapter_fp = adapter_directory_fingerprint(adapter_root)
        if adapter_metadata.get("adapter_fingerprint") != adapter_fp:
            raise MacroContractError("Stage-A LoRA adapter fingerprint mismatch")
    encoder_provenance = {
    ...
    if adapter_fp is not None:
        encoder_provenance.update({
            "semantic_encoder_adapter_schema": str(
                adapter_metadata["adapter_schema"]
            ),
    ...
    if args.lora_adapter is not None:
        try:
            ...
            raise RuntimeError("peft is required to load the Stage-A adapter") from exc
        model = PeftModel.from_pretrained(
            model, args.lora_adapter, is_trainable=False,
        )
    ...
    if manifest_out.exists() and not bool(args.force):
        ...
        expected_provenance = {
            ...
            "semantic_system_prompt_sha256": hashlib.sha256(
        ...
        if existing_manifest.get(
            "semantic_encoder_lora_adapter_fingerprint"
        ) != adapter_fp:
            raise MacroContractError(
                "existing semantic cache LoRA adapter differs; use --force"
            )
    ...
    payload = {
        ...
        "semantic_prompt_schema_version": PROMPT_SCHEMA_VERSION,
        "semantic_system_prompt": SYSTEM_PROMPT,
        "semantic_system_prompt_sha256": hashlib.sha256(
            SYSTEM_PROMPT.encode()
        ).hexdigest(),
        "semantic_pooling_policy": POOLING_POLICY,
    ...
    if adapter_fp is not None:
        payload.update({
            ...
            "semantic_encoder_lora_adapter": str(args.lora_adapter),
            "semantic_encoder_lora_adapter_fingerprint": adapter_fp,
            "semantic_encoder_lora_training_contract": {
                key: adapter_metadata.get(key)
                for key in (
                    "objective", "temperature", "dataset_manifest_fingerprint",
                    "lora", "checkpoint_selection", "step", "validation",
                )
            },
        })
